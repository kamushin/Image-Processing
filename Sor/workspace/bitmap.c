...
int BMP_Write(char *file,BMPImage *bitmap) {
	...
	int count = 0,Width_Stride = (4 - bitmap->image_header->Width *3%4)%4;
	for(i = 0;i < bitmap->image_header->Height;++i) {
		for(j = 0;j < bitmap->image_header->Width;++j) {
			ptr[0] = bitmap->R[count];
			ptr[1] = bitmap->G[count];
			ptr[2] = bitmap->B[count];
			if(count<10)
				printf("%f,%f,%f\n",bitmap->R[count],bitmap->G[count],bitmap->B[count]);
			ptr += 3;
			count++;
		}
		ptr += Width_Stride;
...
static void BMP_SizeEqual(BMPImage *src1,BMPImage *src2) {
...
int BMP_Sub(BMPImage * src1,BMPImage * src2,BMPImage *dst) {
	BMP_SizeEqual(src1,src2);
	BMP_SizeEqual(src2,dst);
	int i,j,count = 0;
...
int BMP_Copy(BMPImage *dst,BMPImage *src) {
	BMP_SizeEqual(dst,src);
	memcpy(dst->R,src->R,sizeof(float)*src->Height*src->Width);
	memcpy(dst->G,src->G,sizeof(float)*src->Height*src->Width);
	memcpy(dst->B,src->B,sizeof(float)*src->Height*src->Width);
}

int BMP_Mul(BMPImage *src1,BMPImage *src2,BMPImage *dst) {
	BMP_SizeEqual(src1,src2);
	BMP_SizeEqual(src2,dst);
	int i,j,count = 0;
	for(i = 0 ; i < src1->Height ;++i) {
...
int BMP_Init(BMPImage *src) {
	memset(src->R,0,sizeof(float)*src->Height*src->Width);
	memset(src->G,0,sizeof(float)*src->Height*src->Width);
	memset(src->B,0,sizeof(float)*src->Height*src->Width);
}

int BMP_Ave2(BMPImage * src1,BMPImage * src2,BMPImage *dst) {
	BMP_SizeEqual(src1,src2);
	BMP_SizeEqual(src2,dst);
	int i,j,count = 0;
	for(i = 0 ; i < src1->Height ;++i) {
		for(j = 0 ; j <src1->Width ;++j) {
			dst->R[count] = sqrt(src1->R[count]*src1->R[count] + src2->R[count]*src2->R[count]);
			dst->G[count] = sqrt(src1->G[count]*src1->R[count] + src2->G[count]*src2->R[count]);
			dst->B[count] = sqrt(src1->B[count]*src1->R[count] + src2->B[count]*src2->R[count]);
			count++;
		}
	}
	return 0;
